...
for CELL in ${CELLLINE[@]}; do
		...
		echo "python $TRAIN --fileInput="\$tdir/LARGE_NETWORK/" --fileOutput=$OUTPUT --parameterLDS --cellLine=\"$CELL\" --chrPair=\"$CHRPAIR\" --model=\"1\" --epochs 100 --batch_size 512 > $LOGS/$CELL\_$CHRPAIR\_1log.out" >> $SLURM/large_$CELL\_$CHRPAIR\-1.slurm

                # Output header
                echo "$HEADER" > $SLURM/large_$CELL\_$CHRPAIR\-2.slurm
                echo "python $TRAIN --fileInput="\$tdir/LARGE_NETWORK/" --fileOutput=$OUTPUT --parameterLDS --cellLine=\"$CELL\" --chrPair=\"$CHRPAIR\" --model=\"2\" --epochs 100 --batch_size 512 > $LOGS/$CELL\_$CHRPAIR\_2log.out" >> $SLURM/large_$CELL\_$CHRPAIR\-2.slurm

                # Output header
                echo "$HEADER" > $SLURM/large_$CELL\_$CHRPAIR\-3.slurm
                echo "python $TRAIN --fileInput="\$tdir/LARGE_NETWORK/" --fileOutput=$OUTPUT --parameterLDS --cellLine=\"$CELL\" --chrPair=\"$CHRPAIR\" --model=\"3\" --epochs 100 --batch_size 512 > $LOGS/$CELL\_$CHRPAIR\_3log.out" >> $SLURM/large_$CELL\_$CHRPAIR\-3.slurm

                # Output header
                echo "$HEADER" > $SLURM/large_$CELL\_$CHRPAIR\-4.slurm
                echo "python $TRAIN --fileInput="\$tdir/LARGE_NETWORK/" --fileOutput=$OUTPUT --parameterLDS --cellLine=\"$CELL\" --chrPair=\"$CHRPAIR\" --model=\"4\" --epochs 100 --batch_size 512 > $LOGS/$CELL\_$CHRPAIR\_4log.out" >> $SLURM/large_$CELL\_$CHRPAIR\-4.slurm

                # Output header
                echo "$HEADER" > $SLURM/large_$CELL\_$CHRPAIR\-5.slurm
                echo "python $TRAIN --fileInput="\$tdir/LARGE_NETWORK/" --fileOutput=$OUTPUT --parameterLDS --cellLine=\"$CELL\" --chrPair=\"$CHRPAIR\" --model=\"5\" --epochs 100 --batch_size 512 > $LOGS/$CELL\_$CHRPAIR\_5log.out" >> $SLURM/large_$CELL\_$CHRPAIR\-5.slurm
