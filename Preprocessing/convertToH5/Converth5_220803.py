...
def compressTrainData(directory="./TRAIN/*", output="./TRAIN/trainData.h5"):
    ...
    for cell in dict.keys():
        for file_type in dict[cell].keys():
            # put the files in the order of types
            ...
            for t in types:
                for f in files:
                    if t in f:
                        sortedData.append(f)
            dict[cell][file_type] = sortedData
            print(f"\n\tcell: {cell}\n\t\tfile_type: {file_type}\n\t\t\tfiles: {dict[cell][file_type]}")
            data = []
            for f in sortedData:
                data.append(pd.read_csv(directory[:-1]+f, sep=" ", header=None))
            data = pd.concat(data, axis=1)

            # append data to hdf5 file as a new dataset
            with h5py.File(output, 'a') as f:
                f.create_dataset(cell + "_" + file_type, data=data.values,compression='gzip', compression_opts=6)
