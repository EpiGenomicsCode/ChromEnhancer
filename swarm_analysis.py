...
def visualize(APSO, gradients, saliency, epoch, name, model, output_path):
    ...
    ig_attr_test = gradients.attribute(torch.tensor(np.array(positions)).float(), internal_batch_size=8000, n_steps=50)
    #print(ig_attr_test)
   ...
   # Sum every 100 values
    ...
    if epoch > 19:
        ...
        np.savetxt(f"{output_path}/{epoch_padded}_{name}_grad_compressed.tsv", compressed_activation, delimiter='\t')
        # Output current swarm saliency to TSV
        np.savetxt(f"{output_path}/{epoch_padded}_{name}_saliency_compressed.tsv", compressed_saliency, delimiter='\t')
