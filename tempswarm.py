...
def main():
    ...
    attack_model_apso_random(model, (1, 500), 500, 20, "./Output/")
...
def attackLoop(inputData: np.ndarray, model, epochs: int, outdir: str, pca = None) -> None:
    ...
    for i in loop:
        ...
        plotimg( apso, i, model)
        # create a gif
        imageio.mimsave("img.gif", [imageio.imread("img_{}.png".format(j)) for j in range(0, i)], fps=.5)
    apso.save_history("temp.csv")


def plotimg(apso, i, model):
    img = []
    value = []
    salency = []
    binData = []
